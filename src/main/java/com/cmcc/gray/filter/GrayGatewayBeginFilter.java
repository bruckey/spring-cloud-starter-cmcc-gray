...
public class GrayGatewayBeginFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
       /* GrayStatusEnum grayStatusEnum = GrayStatusEnum.ALL;
        ...
        String grayTag = headers.getFirst(GrayConstant.GRAY_HEADER);
        // 当灰度开关打开时才进行请求头判断
        GrayFlagRequestHolder.setGrayTag(grayTag);
        ServerHttpRequest newRequest = exchange.getRequest().mutate()
                .header(GrayConstant.GRAY_HEADER, grayTag)
                .build();
